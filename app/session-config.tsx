  ...
  return (
    <Screen>
      <View style={styles.container}>
        ...
        {mode === 'spicy' ? (
          <Card>
            <Text style={[typography.caption]}>{t('config.range')}</Text>
            <Spacer size={spacing.md} />
            <Text style={[typography.body, { color: colors.text }]}>
              {t('config.rangeMin')}: <Text style={{ color: colors.accent }}>{spicyMin} min</Text>
            </Text>
            ...
            <View style={styles.chipRow}>
              {SPICY_MINS.map((v) => (
                <Chip
                  ...
                  label={String(v)}
                  active={spicyMin === v}
                  onPress={() => setSpicyMin(Math.min(v, spicyMax - 1))}
                />
              ))}
            </View>
            <Spacer size={spacing.md} />
            <Text style={[typography.body, { color: colors.text }]}>
              {t('config.rangeMax')}: <Text style={{ color: colors.accent }}>{spicyMax} min</Text>
            </Text>
            <Spacer size={spacing.sm} />
            <View style={styles.chipRow}>
              {SPICY_MAXS.map((v) => (
                <Chip
                  ...
                  label={String(v)}
                  active={spicyMax === v}
                  onPress={() => setSpicyMax(Math.max(v, spicyMin + 1))}
                />
              ))}
            </View>
          </Card>
          ...
          <Card>
            <Text style={[typography.caption]}>{t('config.duration')}</Text>
            <Spacer size={spacing.md} />
            <View style={styles.chipRow}>
              {DURATIONS.map((d) => (
                <Chip
        ...
        {mode === 'simple' && (
          <>
            <Spacer size={spacing.md} />
            <Card>
              <Text style={[typography.caption]}>{t('config.sound')}</Text>
              <Spacer size={spacing.md} />
              <View style={styles.chipRow}>
                {SOUNDS.map((key) => (
                  <Chip
                    ...
                    active={sound === (key === 'none' ? null : key)}
                    onPress={() => setSound(key === 'none' ? null : key)}
                  />
                ))}
              </View>
            </Card>
